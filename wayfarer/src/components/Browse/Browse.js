...
import cityPosts from "../../model/cityPosts";

class Browse extends Component {
  state = {
    posts: [],
    cities: ["San Francisco", "London", "Paris", "Tokyo"],
    city: null
  };
  ...
  citySelect = event => {
    ...
    this.setState({ city: event.target.id, image_url: event.target.image_url });
  };
  ...
  render() {
    ...
    let cities = this.state.cities.map(city => {
      <li onClick={this.citySelect}>
        <img id={city._id} src={city.image_url} alt="img" />
      </li>;
    });
